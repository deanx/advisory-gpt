...
def add_project():
    ...
    with sqlite3.connect("projects.db") as connection:
        cursor = connection.cursor()
        cursor.execute(
            "insert into projects (username, project) values ('"+username+"','"+new_project+"');")
        os.mkdir("./embeddings/" + username + "/" + new_project, 0o777)
        os.mkdir("./user_data/" + username + "/" + new_project, 0o777)
        st.toast("Project added")


st.text_input(label="new project", label_visibility="hidden",
              placeholder="New project name", key="new_project", on_change=add_project)
